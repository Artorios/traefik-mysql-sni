...
type RouterFactory struct {
	...
	entryPointProtocols map[string]string // протоколы entrypoint'ов
...
func (f *RouterFactory) CreateRouters(rtConf *runtime.Configuration) (map[string]*tcprouter.Router, map[string]udp.Handler) {
	...
	log.Debug().Strs("entryPointsTCP", f.entryPointsTCP).Msg("Building TCP handlers")
	routersTCP := rtTCPManager.BuildHandlers(ctx, f.entryPointsTCP)
	log.Debug().Int("routerCount", len(routersTCP)).Msg("Built TCP handlers")
